...
(deftest test-reporting
  (let [out (:output (test-run-tests 's-exp.eftest.test-ns.single-failing-test))]
    (is (re-find #"FAIL in s-exp.eftest.test-ns.single-failing-test/single-failing-test" out))
...
(deftest test-slow-test-report
  (testing "should fail with an accurate var location"
    (let [out (:output
               (test-run-tests ['s-exp.eftest.test-ns.slow-test] {:test-warn-time 5}))]
      (is (re-find #"LONG TEST in s-exp.eftest.test-ns.slow-test/a-slow-test\n" out)))))
